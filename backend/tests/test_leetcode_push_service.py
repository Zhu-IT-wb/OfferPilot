...
def test_leetcode_pushes_morning_once_and_reminds_only_pending_at_night() -> None:
    class FakeMessageService:
        ...
        async def send_text_message(self, receive_id, text, receive_id_type="open_id"):
            self.messages.append((receive_id, text))
...
def test_leetcode_push_stops_retrying_after_three_persisted_failures() -> None:
    ...
    assert repository.get_delivery_attempts("feishu:ou_1", now.date(), "morning") == 3
    assert repository.has_delivery("feishu:ou_1", now.date(), "morning") is False
